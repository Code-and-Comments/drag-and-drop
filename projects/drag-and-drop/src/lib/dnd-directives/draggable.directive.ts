...
import { DndItemState, dragabbleZ } from '../dnd';
...
export class DraggableDirective<Item extends object> implements OnInit, OnDestroy {
  ...
  @HostBinding(`class.${defaultDndCssConfig.dragIsDragging}`) private currentElementIsDragging = false;
  private destroy$ = new Subject<void>()

  private dndService = inject(DndService<Item>);
  private elRef = inject(ElementRef);
  private itemState: Observable<DndItemState>
  ...
  */
  ...
  ngOnInit() {
    this.itemState = this.dndService.createItemStateObservable(this.dndItem);
    this.itemState.pipe(
